...
export default function OnboardingSubjectsPage() {
  ...
  useEffect(() => {
    if (!loading && profile?.onboardingComplete && profile.activeSubjects.length > 0) {
      router.replace('/quiz/subjects');
      return;
    }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900">
        <div className="text-center">
          ...
          <p className="mt-6 text-white/60 font-medium">Loading your onboarding...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 px-4 py-10">
      <div className="max-w-3xl mx-auto">
        <header className="mb-10">
          <p className="text-purple-200/80 uppercase tracking-[0.2em] text-xs">Onboarding</p>
          <h1 className="text-3xl sm:text-4xl font-bold text-white mt-3">
            Pick your active subjects
          </h1>
          <p className="text-white/60 mt-3 max-w-xl">
            Choose at least one subject you want to study. We will use these to tailor your daily
            quizzes.
          </p>
        </header>
